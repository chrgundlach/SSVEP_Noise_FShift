...
hold on
plot(EEG_rsfS2.times, squeeze(mean( EEG_rsfS2ep_ITPC(pl.elec2plot_i,:,:),1)));
xlim([-200 700]); 
xlabel('latency');
ylabel('ITPC');
legend({'SSVEP1';'SSVEP2'});
title(sprintf('inter-trial phase clustering | [%1.1f %1.1f]s | at %s',p.ep_time,vararg2str(pl.elec2plot)))


nexttile
plot(EEG_rsfS1.times, squeeze(mean( EEG_rsfS1ep_specCoh(pl.elec2plot_i,:,:),1)));
hold on
plot(EEG_rsfS2.times, squeeze(mean( EEG_rsfS2ep_specCoh(pl.elec2plot_i,:,:),1)));
xlim([-200 700]); 
xlabel('latency');
ylabel('specCoh');
legend({'SSVEP1';'SSVEP2'});
title(sprintf('magnitude-squared coherence | [%1.1f %1.1f]s | at %s',p.ep_time,vararg2str(pl.elec2plot)))